    ...
    if (this.lastClicked != null && !this.lastClicked.fueJugada()) {
      ...
      if (posX < this.xEnLinea + 4) {
        ...
        if (this.tablero.chequearGanador(this.lastClicked)) {
          this.mostrarGanador(this.lastClicked)
          finalizarJuego()
          
        }
        this.jugador1.setTurno(this.jugador2.esSuTurno());
        this.jugador2.setTurno(!this.jugador1.esSuTurno());
        ficha1.classList.toggle("ocultarFichaJugador");
        ficha2.classList.toggle("ocultarFichaJugador");
      }
    }
  }

  mostrarGanador(fichaGanador) {
    finJuego.classList.toggle("fin-juego-ocultar");
    finJuego.innerHTML += `<div><h4>Ganador: jugador  ${fichaGanador.getJugador().getNombre()}</h4></div>`;
    let btnVolverJugar = document.querySelector("#btnVolverJugar");
